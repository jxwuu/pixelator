...
TEST_CASE("stats::basic sums","[weight=1][part=stats]"){
    PNG data; data.resize(2,2);
    for (int i = 0; i < 2; i ++){
        for (int j = 0; j < 2; j++){
            RGBAPixel * p = data.getPixel(i,j);
...
TEST_CASE("qtcount::basic ctor render","[weight=1][part=qtcount]"){
    ...
    qtcount t1(img);
    PNG out = t1.render();
    out.writeToFile("images/orig/geo copy.png"); 
    REQUIRE(out==img);
}
...
TEST_CASE("qtcount::basic prune","[weight=1][part=qtcount]"){
    ...
    result.writeToFile("images/orig/adaaaaaa.png");
...
// TEST_CASE("qtcout::basic pruneSize","[weight=1][part=qtcount]"){
//     PNG img;
//     img.readFromFile("images/orig/adasquare.png");
...
TEST_CASE("qtcount::baaic idealPrune","[weight=1][part=qtcount]"){
    PNG img;
    img.readFromFile("images/orig/dogdogodgdoggod.png");
    
    qtcount t1(img); 
    t1.prune(8000);
    PNG result1 = t1.render(); 
    result1.writeToFile("images/orig/qtdogcount.png");

    qtvar t2(img); 
     t2.prune(8000);
     PNG result2 = t2.render(); 
    result2.writeToFile("images/orig/qtdogvar.png");
...
TEST_CASE("qtvar::basic prune","[weight=1][part=qtvar]"){
    ...
    result.writeToFile("images/orig/avar.png");
